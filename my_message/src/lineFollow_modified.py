...
class Visualiser:
    ...
    def update_plot(self,frame):
        ...
        if x_index <self.maxt/4 or np.sin((x_index2/fs)*2*np.pi*f/100)*6>0:
        ...
        if x_index>self.maxt/2:
            dif = self.y_data[x_index] - self.y_desired_data[int(x_index-self.maxt/4)]
        else:
            dif=0
        squared_dif = dif**2
